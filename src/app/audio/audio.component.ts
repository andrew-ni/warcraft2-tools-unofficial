...
export class AudioComponent implements OnInit {
  ...
  changeAudio() {
    dialog.showOpenDialog(options, (paths: string[]) => {
      ...
      this.soundService.copyFile(paths[0], path.join(this.mapService.resourcePath, 'data', this.destPath), this.selectedCategory, this.selectedClipName, newClip);
    });
  }
  ...
  revertAudio() {
    ...
    const orig = new Audio(path.join(this.mapService.resourcePath, 'data', 'snd', name));
